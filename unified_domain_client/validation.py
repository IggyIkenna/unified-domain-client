...
class DomainValidationService:
    ...
    def _validate_internal_timestamp_ordering(
        ...
        if invalid_count > 0:
            warnings.append(
                ...
                " (expected: timestamp_out >= timestamp)"
            )
        ...
        if invalid_count > 0:
            warnings.append(
                ...
                " (expected: timestamp_in <= timestamp_out)"
            )
    ...
    def _validate_timestamp_ordering(self, data: pd.DataFrame) -> ValidationResult:
        ...
        if not is_sorted:
            ...
            return ValidationResult(
                ...
                valid=False,
                errors=[
                    f"{out_of_order_count} out-of-order timestamps found"
    ...
    def _validate_feature_completeness(self, data: pd.DataFrame) -> ValidationResult:
        ...
        if len(high_nan_cols) > 0:
            warnings.append(
                f"{len(high_nan_cols)} feature columns with >10% NaN values:"
                f" {list(high_nan_cols.index)}"
            )
